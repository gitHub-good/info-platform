 ...
 *
 * <p>从 {@code Authorization: Bearer <token>} 解析并校验 access 令牌，校验通过则把 {@link UserContext.Principal}（含
 * userId）写入 ThreadLocal，供 T11 watchlist 行级权限取数。 白名单（登录/换发/actuator）直接放行；令牌缺失/无效/过期 → 401 + 1003（统一
 * Result 体）。
 */
@Component
