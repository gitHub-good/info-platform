...
/**
 * GLM / 智谱 provider adapter（T19 fallback provider，Spike-2 §2.2 + §3；T35 改运行时解析）。
 *
 ...
 * AbstractOpenAiCompatProvider}，差异仅在身份；model/api-key/baseUrl 每次调用经 {@link ConfigCenter}
 * 解析当前值（页面保存即生效，baseUrl 重启生效）。
 *
...
public class GlmAdapter extends AbstractOpenAiCompatProvider {
    ...
    public GlmAdapter(RestClient.Builder restClientBuilder, ConfigCenter configCenter) {
