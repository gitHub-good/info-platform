 ...
 * expireAfterWrite(24h)} 只是从首次写入起算的近似日重置（T19 注释明示「严格跨日重置留 T30」）， 换日后取新键计数自然归零，实现严格跨日重置；
 * 旧键条目由 {@code expireAfterWrite(48h)} 兜底逐出（覆盖单日最长写入跨度，只作清理不影响计数）。 日界取 {@link
 * Clock} 系统时区（对齐用户「当日」直觉），测试经构造注入 {@code Clock.fixed} 模拟换日。
 *
 ...
 * 计入实际用量，并在<b>首次越过告警线</b>（{@code 预算 × budgetWarnRatio}，默认 0.8）时记一次 WARN（跨线一次为限，不逐次刷屏），
 * 留痕表另落 REJECTED 行（T30 报表告警状态同源本阈值）。
 *
 * <p>实现领域端口 {@link LlmCostBudget}（预检 + 预算策略查询），供应用层 {@code AIBriefService} POST 预检注入端口 而非本实现类（守护分层：避
 * application↔infrastructure 循环依赖）；计费入账由 {@code LlmGatewayImpl} 调本类 {@link #recordUsage} 完成。
 *
 * <p>check/record 间存在 TOCTOU 窗口（个人量级可接受）：单次简报 ~4900 token / 预算 20000， 单次不会大幅越界，越界由下次调用拦截。计数为内存态，重启清零（严格跨日的权威口径以
 * llm_call_log 报表为准，观测与执行分离）。
 *
...
public class LlmCostGuard implements LlmCostBudget {
    ...
    /**
     * 计入实际用量（按 {@code usage.totalTokens}）；{@code userId <=0} 或 usage null 跳过。 首次越过告警线（{@code
     * 预算 × budgetWarnRatio}）记一次 WARN。
     */
