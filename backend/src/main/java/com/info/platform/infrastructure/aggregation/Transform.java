 ...
 * {@code to_iso_date} / {@code to_iso_datetime} / {@code epoch_seconds_to_iso} / {@code strip_html}），
 * {@link #from} 大小写不敏感解析，缺失或空串视作 {@link #NONE}。
 *
 * <p>M13 新增三值（方案 §4.3，资讯源通用 JSON 引擎用；既有六源映射零影响）： {@code to_iso_datetime}（{@code yyyy-MM-dd
 * HH:mm[:ss]} 墙钟时间按 Asia/Shanghai → ISO-8601 UTC 秒）/ {@code epoch_seconds_to_iso}（Unix 秒 → ISO-8601 UTC）/
 * {@code strip_html}（jsoup 抽纯文本，richtext 源用）。
 */
