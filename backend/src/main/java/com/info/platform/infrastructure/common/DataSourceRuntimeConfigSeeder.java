 ...
 * 代码内置缺省，application.yml {@code adapter:} 段已删）。
 *
 ...
 * adapter.mock.enabled=true} 的语义平移）；enabled 恒 true。
 */
@Component
public class DataSourceRuntimeConfigSeeder implements RuntimeConfigSeeder {
    ...
    private RuntimeConfigSeed seedOf(SourceCode code) {
        Map<String, Object> doc =
                Map.of(
                        "enabled", true,
                        "mode", DataSourceDefaults.DEFAULT_MODE.name(),
                        "timeoutMillis", DataSourceDefaults.timeoutMillis(code),
                        "retries", DataSourceDefaults.RETRIES_NONE,
                        "cacheTtlSeconds", DataSourceDefaults.cacheTtlSeconds(code),
                        "failureCacheTtlSeconds", DataSourceDefaults.failureCacheTtlSeconds(code),
                        "params", DataSourceDefaults.params(code));
