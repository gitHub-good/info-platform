 ...
 *
 * <p>受理前端埋点（标的详情页/政策详情/简报页阅读事件）并落 {@code reading_event}：解析标的关联 （subjectId 优先、subjectCode 次之，均解析不到记
 * null 不拒收——政策/每日推荐阅读本就无单一标的）， 按「同 user + contentType + contentRef 且 {@link #DEDUP_WINDOW} 内已存在」去重后
 * INSERT。
 *
...
public class ReadingEventService {
     ...
     * @param contentTypeName 内容类型名（SUBJECT_DETAIL/POLICY/AI_BRIEF）
     * @param contentRef 内容引用（标的代码/政策 id/简报 taskId）
     * @param subjectCode 标的代码（可空；详情页埋点传入）
    ...
    private static ReadingEventType parseType(String contentTypeName) {
        try {
            ...
            throw new BusinessException(
                    ErrorCode.PARAM_INVALID, "contentType 取值 SUBJECT_DETAIL/POLICY/AI_BRIEF");
        }
    }
