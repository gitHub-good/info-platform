...
/**
 * 留痕表枚举白名单（T70，ADR-0036 §3）——可被清理的唯一四张旁路留痕表，兼表级常量的<b>单一事实源</b>：
 * 物理表名 / 配置字段名 / 缺省窗口 / 下限被校验器（保存侧）、解析器（执行侧回退）、删除端口、种子共用。
 *
 * <p>枚举序即留痕明细段序与清理轮遍历序（固定，SUCCESS 行 error_message 四段格式由此确定）。
 *
 * <table border="1">
 *   <caption>表级常量（技术方案 §4.1）</caption>
