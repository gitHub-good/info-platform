 ...
 *
 * <p>实际计费入账（{@code recordUsage}）与预算计数由基础设施层 {@code LlmGatewayImpl} 在 chat 时调用 {@code
 * LlmCostGuard} 完成。T30 起端口追加预算策略查询（{@link #dailyBudgetTokens}/{@link #budgetWarnRatio}）， 供成本报表
 * {@code LlmCostReportService} 展示上限余量与告警状态（执行与观测同源，单一事实）。
 */
public interface LlmCostBudget {
    ...
    /**
     * 预算告警阈值比例（0~1，配置 {@code llm.budget-warn-ratio}）： 用量达 {@code dailyBudgetTokens ×
     * 本值} 即告警（WARNING 状态 + WARN 日志），耗尽（100%）才拦截。
     */
