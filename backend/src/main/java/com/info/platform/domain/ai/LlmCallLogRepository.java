 ...
 *
 * <p>领域层纯净接口，不依赖 MyBatis/Spring 等框架类型。写入方为基础设施层 {@code LlmCallLogger}（封装 {@code LlmGatewayImpl}
 * 的留痕落库，失败仅记日志不阻断调用）； 查询方为应用层 {@code LlmCostReportService} （成本报表窗口聚合，经接口层 {@code
 * LlmCostReportController} 暴露）。
 */
public interface LlmCallLogRepository {
     ...
     *
     * <p>成本报表窗口取数：{@code start} 为窗口起点（今日零点 / 近 N 天），个人量级窗口行数有限， {@code limit} 仅作上限护栏（超出按最新 limit
     * 条聚合，成本口径截断在报表页脚注说明）。
     *
     * @param start 窗口起点（含）
     * @param limit 返回条数上限
