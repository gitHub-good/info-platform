...
/**
 * 降级链解析（ADR-0033）：DB 配置链优先，代码内置默认链只做兜底；旧单值开关读取兼容折算。
 *
 ...
 *       注册表全链；单值（如 {@code tencent}）→ 单元素链（强制单源语义）
...
public final class FallbackChains {
    ...
    public static List<String> resolve(
            List<String> rawChain, String legacyValue, List<String> registry) {
        if (rawChain != null) {
            if (rawChain.isEmpty()) {
                return List.of(registry.get(0));
            }
            return isValidChain(rawChain, registry) ? List.copyOf(rawChain) : List.copyOf(registry);
        }
        ...
        return registry.contains(normalized) ? List.of(normalized) : List.copyOf(registry);
    }

    /** chain 合法性：成员 ∈ 注册表且无重复（空链合法 = 仅主源；null 视为未配置不参与判断）。 */
    public static boolean isValidChain(List<String> chain, List<String> registry) {
        if (chain == null) {
            return true;
        }
        if (chain.isEmpty()) {
        ...
        if (chain.stream().anyMatch(provider -> !registry.contains(provider))) {
            return false;
        }
        return chain.stream().distinct().count() == chain.size();
    }

    /** 旧单值开关合法性：空白（= 缺省 auto）/{@code auto}/注册表成员（大小写不敏感）。 */
    public static boolean isValidLegacy(String raw, List<String> registry) {
        if (raw == null || raw.isBlank()) {
            return true;
        }
        String normalized = raw.trim().toLowerCase(Locale.ROOT);
        return AUTO.equals(normalized) || registry.contains(normalized);
    }
}
