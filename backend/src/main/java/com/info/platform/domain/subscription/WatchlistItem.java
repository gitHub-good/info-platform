 ...
 *
 * <p>领域层纯净：仅依赖 JDK 类型（含 {@link BigDecimal} 表达异动阈值），可脱离容器单测、可移植。 持久化字段（id/version/时间戳）由基础设施层
 * {@code WatchlistRepositoryImpl} 经 {@link #reconstruct} 回填。
 *
 * <p>异动阈值默认 {@link #DEFAULT_THRESHOLD}（3.00%），与 DDL {@code DEFAULT 3.00} 对齐； DECIMAL(5,2) 映射 {@link BigDecimal} 避免浮点精度损失。
 */
public class WatchlistItem {
    ...
    public static WatchlistItem create(Long watchlistId, Long subjectId, BigDecimal anomalyThreshold) {
        ...
        item.anomalyThreshold =
                anomalyThreshold != null ? anomalyThreshold : DEFAULT_THRESHOLD;
